                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group  col-md-6 shangpinCommentbackContentDiv">
                                        <label>评价内容</label>
                                        <input id="shangpinCommentbackContentupload" name="file" type="file">
                                        <script id="shangpinCommentbackContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var shangpinCommentbackContentUe = UE.getEditor('shangpinCommentbackContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="shangpinCommentbackContent" id="shangpinCommentbackContent-input">
                                    </div>
                                    <div class="form-group  col-md-6 replyContentDiv">
                                        <label>回复内容</label>
...
<script>
    ...
    function upload() {
        ...
        $('#shangpinCommentbackContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('shangpinCommentbackContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.shangpinCommentbackContent != null && ruleForm.shangpinCommentbackContent != 'null' && ruleForm.shangpinCommentbackContent != '' && $("#shangpinCommentbackContentupload").length>0) {

            var shangpinCommentbackContentUeditor = UE.getEditor('shangpinCommentbackContentEditor');
            shangpinCommentbackContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.shangpinCommentbackContent != null){
                    mes = ''+ ruleForm.shangpinCommentbackContent;
                    // mes = mes.replace(/\n/g, "<br>");
                }
                shangpinCommentbackContentUeditor.setContent(mes);
            });
        }
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#shangpinCommentbackContentupload").length>0) {
            var shangpinCommentbackContentEditor = UE.getEditor('shangpinCommentbackContentEditor');
            if (shangpinCommentbackContentEditor.hasContents()) {
                $('#shangpinCommentbackContent-input').attr('value', shangpinCommentbackContentEditor.getContent());
            }
        }
        if($("#replyContentupload").length>0) {
            var replyContentEditor = UE.getEditor('replyContentEditor');
            if (replyContentEditor.hasContents()) {
                $('#replyContent-input').attr('value', replyContentEditor.getContent());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                shangpinId: "required",
                yonghuId: "required",
                shangpinCommentbackContent: "required",
                replyContent: "required",
            },
            messages: {
                shangpinId: "商品不能为空",
                yonghuId: "用户不能为空",
                shangpinCommentbackContent: "评价内容不能为空",
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#shangpinCommentbackContent").val(ruleForm.shangpinCommentbackContent);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            var shangpinCommentbackContentUeditor = UE.getEditor('shangpinCommentbackContentEditor');
            shangpinCommentbackContentUeditor.ready(function () {
                shangpinCommentbackContentUeditor.setDisabled('fullscreen');
            });
