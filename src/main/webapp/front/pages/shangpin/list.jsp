            ...
            <div class="ng-inmod-ini">
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="shangpinKucunNumberStart" id="shangpinKucunNumberStart"  lay-skin="integer"   placeholder="商品库存开始" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <div class="layui-input-inline" style="margin-top: -5px">
                              <input type="text" name="shangpinKucunNumberEnd"   id="shangpinKucunNumberEnd"    lay-skin="integer|shangpinKucunNumberEnd"   placeholder="商品库存结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
                          <button id="btn-search" type="button" class="layui-btn" style="background-color: var(--publicColor,orange);">
                              搜索
                          </button>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../shangpin/detail.jsp?id='+item.id)">
                                        <a href="#" class="ng-prsliik">
                                            <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                                <img class="swiper-item" :src="item.shangpinPhoto?item.shangpinPhoto.split(',')[0]:''" />
                                            </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../shangpin/detail.jsp?id='+item.id)"><span>{{item.shangpinName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.shangpinNewMoney"><span style="color: red;">{{item.shangpinNewMoney}} RMB</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
        ...
        methods: {
            ...
            ,deleteShangpin(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    ...
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('shangpin/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        // 是否特卖的查询
        function temaiTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=temai_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.temaiTypesList = res.data.list;
                }
            });
        }
        // 是否上架的查询
        ...
        form.verify({
            ...
            ,shangpinKucunNumberEnd: function(value, item){ //value：表单的值、item：表单的DOM对象
                    var shangpinKucunNumberStart = $("#shangpinKucunNumberStart").val();
                 if(shangpinKucunNumberStart != null && shangpinKucunNumberStart != "" && shangpinKucunNumberStart != "null"){
                    var number = parseInt(value)- parseInt(shangpinKucunNumberStart);
                    if(number<0){
                        return '商品库存的结束值不能小于开始值';
                    }
                 }
            }
        ...
        function pageList() {
            ...
            if (jquery('#shangpinKucunNumberStart').val() != null && jquery('#shangpinKucunNumberStart').val() != "" && jquery('#shangpinKucunNumberStart').val() != "null" ) {
                param['shangpinKucunNumberStart'] = jquery('#shangpinKucunNumberStart').val();
            }
            if (jquery('#shangpinKucunNumberEnd').val() != null && jquery('#shangpinKucunNumberEnd').val() != "" && jquery('#shangpinKucunNumberEnd').val() != "null" ) {
                param['shangpinKucunNumberEnd'] = jquery('#shangpinKucunNumberEnd').val();
            }
