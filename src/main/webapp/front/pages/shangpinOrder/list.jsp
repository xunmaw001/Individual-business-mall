                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            ...
                            <td style="width: 80px;">{{item.shouhuoValue}}</td>
                            <td style="width: 80px;">{{ item.shouhuoTypes!=2?item.fahuodanwei:"自取"}}</td>
                            <td style="width: 80px;">{{ item.shouhuoTypes!=2?item.fahuoren:"自取"}}</td>
                            <td style="width: 80px;">{{ item.shouhuoTypes!=2?item.lianxifangshi:"自取"}}</td>
                            ...
                            <td style="width: 100px;">
                                ...
                                <button v-if="item.shangpinOrderTypes==4" @click="pay(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius btn-theme">
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            ,pageList(shangpinOrderTypes) {
                ...
                // 获取列表数据
                layui.http.request('shangpinOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
            ...
            ,submitShangpinCommentback(){
                ...
                layui.http.request("shangpinOrder/commentback?id="+id+"&shangpinCommentbackContent="+shangpinCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function shouhuoTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shouhuo_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.shouhuoTypesList = res.data.list;
                }
            });
        }
        // 支付类型的查询
